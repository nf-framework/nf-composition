...
async function getShowMethodData(context) {
    try {
        ...
        let query = `
            select ${columns.join(', ')}
              from ${meta.view_name}
             where ${where_sql.join('\n and ')}
        `;
        const data = await dbapi.query(query, args.filters, { context })
...
async function saveUnitData(context) {
    ...
    try {
        ...
        await connect.begin();
        const res = await connect.broker(`${args.unitcode}.${args.action}`, args.saveData);
        await connect.commit();

        context.send({ data: { success: true, id: res.data.id } });
    } catch (error) {
